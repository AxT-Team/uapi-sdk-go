...
func New(baseURL, token string) *Client {
	return NewWithOptions(baseURL, token, nil)
}

func NewWithOptions(baseURL, token string, opts *ClientOptions) *Client {
	client := &Client{ baseURL: normalizeBaseURL(baseURL), token: token, cli: &fasthttp.Client{} }
	if opts != nil {
		client.disableCache = opts.DisableCache
	}
	return client
}
...
func (c *Client) applyCacheControl(method string, q map[string]string, disableCache *bool) map[string]string {
	...
	effectiveDisableCache := c.disableCache
	if disableCache != nil {
		effectiveDisableCache = *disableCache
	}
	...
	q["_t"] = fmt.Sprint(time.Now().UnixMilli())
	return q
}

func (c *Client) do(method, path string, q map[string]string, body any, disableCache *bool) (any, error) {
	url := c.baseURL + path
